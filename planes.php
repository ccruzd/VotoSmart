  ...
  <body>
    ...
    <div class="page">
      ...
      <section style="background-image: url('img/fondos/palacio.jpg');" class="header-section fading-title parallax">
        <div class="section-shade sep-top-3x sep-bottom-2x">
          <div class="section-title text-upper text-center upper light">
            ...
            <h2 class="small-space">Planes de <span class="text-primary">Gobierno</span></h2>
          </div>
        </div>
      </section>
	  <!-- End header Section ------------------------------------------------------------------------->
      ...
      <section id="presidente" class="sep-bottom-2x">
        <div class="container">
          <div class="row">
          <?php $query = " SELECT partidos.id, partidos.nombre, partidos.link_foto_partido, candidatos.nombres, candidatos.apellidos
			FROM partidos, candidatos 
			WHERE partidos.id = candidatos.idpartido group by partidos.nombre asc";
          $rs = mysql_query($query);
          while($rsconsulta = mysql_fetch_array($rs)) {
      ?>
            <div class="col-md-4 sep-top-2x" style="height:380px">
              <div class="icon-box icon-horizontal icon-lg">
			    <a href="plan.php?id=<?php echo $rsconsulta['id'] ?>" >
				<img src="<?php echo $rsconsulta['link_foto_partido'] ?>" />
                <div class="icon-box-content">
                  <h5 class="upper"><?php echo $rsconsulta['nombre'] ?></h5>
                  <p><?php echo $rsconsulta['nombres']." ".utf8_encode($rsconsulta['apellidos']) ?></p>
                </div>
				</a>
              </div>
            </div>
