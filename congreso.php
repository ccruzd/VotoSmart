...
<head>
  ...
  <body>
    ...
    <div class="page">
      ...
      <section style="background-image: url('img/fondos/congreso.jpg');" class="header-section fading-title parallax">
        <div class="section-shade sep-top-3x sep-bottom-2x">
          <div class="section-title text-upper text-center upper light">
			<img src="http://votoinformado.pe/voto/img/img_titCongreso.png" />
            <h2 class="small-space">Candidatos al <span class="text-primary">Congreso</span></h2>
          </div>
        </div>
      </section>
	  <!-- End header Section ------------------------------------------------------------------------->
      ...
      <section id="presidente" class="sep-bottom-2x">
        <div class="container">
          <div class="row">
            <div class="col-md-6 col-md-offset-3 sep-top-2x">
				<p class="lead wow flipInX text-center">Selecciona una Región para conocer a los Candidatos</p>
            ...
            <div class="row">
				<div class="col-md-10 col-md-offset-1 sep-top-md text-center">
				  <p class="lead wow flipInX text-center">Selecciona un Partido</p>
				  <?php $query = " SELECT * FROM partidos order by nombre asc";
				  $rs = mysql_query($query);
				  while($rsconsulta = mysql_fetch_array($rs)) {
				  ?>
				  <a onclick="partido_select(<?php echo $rsconsulta['id']?>)">
					<img src="<?php echo $rsconsulta['link_foto_partido'] ?>" style="width:50px;height:50px;"/>
				  </a>
				  <?php } ?>
				</div>
			</div>
    ...
    <script type="text/javascript">
        function partido_select(val){
          region = $("#departamentos").val();
           $.ajax({
             type: 'post',
             url: 'listar_congresistas.php',
             data: {get_option:val, region:region},
